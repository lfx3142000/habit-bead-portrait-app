...
1. [x] Confirmed `AndroidManifest.xml` has no unnecessary exported components (only MainActivity with LAUNCHER intent filter).
2. [x] Confirmed `android:allowBackup="false"` is set.
3. [x] Confirmed `android:usesCleartextTraffic="false"` is set.
4. [x] Confirmed no runtime permissions are added without a documented reason.
5. [ ] Confirm release builds are signed with Play upload credentials, not the debug keystore.
6. [x] Confirmed ProGuard/R8 release build is configured with `isMinifyEnabled = true` and `isShrinkResources = true`.
7. [x] Confirmed no habit names, subtitles, dates, or counts are logged (0 Log.d calls in Kotlin source).

## Performance Validation

Note: The app supports a maximum of 20 habits by design. Users will not encounter larger datasets.
